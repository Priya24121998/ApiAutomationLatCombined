...
<body>
	<div class="header">
		<h1>Order History</h1>
		<a href="b2cSubExportOrderDetails" class="export-button">Export to CSV</a>
	</div>
	<table>
		<thead>
			<tr>
		...
		<tbody>
			<c:forEach var="order" items="${b2cSuborderHistory}">
				<tr>
					<td>${order.id}</td>
					<td>${order.env}</td>
					<td>${order.store}</td>
					...
					<td>${order.rentalType}</td>
					<td>${order.rentalIsbn}</td>
