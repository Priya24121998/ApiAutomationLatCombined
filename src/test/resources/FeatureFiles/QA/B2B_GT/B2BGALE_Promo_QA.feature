Feature: B2B GT Promo Code Scenarios
 ...
 @B2BGT_Promo_QA
  ...
  @B2BGT_NewDelMode_PromoCode_QA
  Scenario:[GDC-12976] [B2B GT- Delivery mode selected -GD] Verify if we can able to place a B2B Order using 100% promo code - DISC100+PO
  ...
  @B2BGT_NewDelMode_PromoCode_QA
  Scenario:[GDC-12976] [B2B GT- Delivery mode selected -GD] Verify if we can able to place a B2B Order using 100% promo code- DISC100+PO+CC
    Given I Hit Api to create B2BGTTwo Token
    When I hit API to create B2BGTTwo cart
      | basestore | cengage-b2b-gt |
      | status    |            201 |
