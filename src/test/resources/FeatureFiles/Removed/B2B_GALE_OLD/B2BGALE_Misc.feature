Feature: B2B GT Misc API Scenarios
  ...
  Scenario: [GDC-10378] User is able to get user profile deatils using API
    Given I Hit Api to create B2BGT Token
    ...
    Then I verify following details in API response
      | accountStatus | ACTIVE                 |
      | storeName     | CATHOLIC DISTANCE UNIV |
      | roles         | not null               |
      | orgUnit       | not null               |
    And Get the appropriate B2BGale customerId in API response
  ...
  Scenario: [GDC-10187] User is able to fetch customer account details using API
    ...
    And I will hit get B2BGale customer account details API
      ...
      | status    |            200 |
    Then I verify following details in API response
      | billToAccounts[0].accountNumber            | 375223 |
      | customerDefaultBillToAccount.accountNumber | 375223 |
      | customerDefaultShipToAccount.accountNumber | 375223 |
      | shipToAccounts[0].accountNumber            | 375223 |
    And Get the appropriate B2BGale customerId in API response
