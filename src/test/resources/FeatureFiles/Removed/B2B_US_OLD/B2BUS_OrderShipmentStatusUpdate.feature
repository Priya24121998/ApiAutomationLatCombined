Feature: B2BUS_OrderShipmentStatusUpdate

@B2BUS_OrderShipmentStatusUpdate
    Scenario: [GDC-12927] Verify Order status was moved to completed status once all the products in the order were shipped                 
    ...
    @B2BUS_OrderShipmentStatusUpdate
    Scenario: [GDC-12930 ] Verify total shipped quantity in a different consignment entry Created
    Given I Hit Api to create B2BUS Token
    Given I Hit Api to create B2BUS Token
    When I hit API to create B2B cart
      | basestore | cengage-b2b-us |
   ...
   @B2BUS_OrderShipmentStatusUpdate
    Scenario: [GDC-12932 ] Verify if shipping quantity more than the ordered quantity error message is displayed
    Given I Hit Api to create B2BUS Token
    When I hit API to create B2B cart
      | basestore | cengage-b2b-us |
      | status    |            201 |
    ...
    And Test Data for GDC-12670 and Order details
     ...
     @B2BUS_OrderShipmentStatusUpdate
    Scenario: [GDC-12936 ] Verify if cancelled few quantity from the ordered quantity, user should be able to ship the remaining quantity
    Given I Hit Api to create B2BUS Token
    When I hit API to create B2B cart
      | basestore | cengage-b2b-us |
      | status    |            201 |
    ...
    @B2BUS_OrderShipmentStatusUpdate
    Scenario: [GDC-12938 ] Verify if BackOrdered few quantity from the ordered quantity,later user should be able to ship ordered quantity when available
    Given I Hit Api to create B2BUS Token
    When I hit API to create B2B cart
      | basestore | cengage-b2b-us |
      | status    |            201 |
